...
def sobol_variance_mc(f, dim, y, N=10000, monitor=False):
    """
    Compute Sobol variances D_y and D_y^\mathrm{tot} of the function f of dim variables
...
if __name__ == '__main__':
    ...
    def f(x):
        return sobol_g_function(x, a)

    D, D_y, D_y_tot = np.zeros(dim), np.zeros(dim), np.zeros(dim)
    for i in range(dim):
        print('Dimension %d' % (i + 1,))
        N, f0, D[i], D_y[i], D_y_tot[i] = sobol_variance_mc(
            f, dim, [i], N=1000, monitor=True
        )
