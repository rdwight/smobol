...
def re_infile(filename, restr, group):
    ...
    if not os.path.isfile(filename):
        print('re_infile(): File not found.', file=sys.stderr)
        return None
    value = []
    fh = open(filename, "r")
...
def toarray(a):
    ...
    if getattr(a, '__iter__', False):
        return np.array(a)  # List case
    else:
        return np.array([a])  # Scalar case
...
def tomat(a):
    ...
    if a.ndim == 1:
        ...
        raise ValueError("Arg must be a numpy array of ndim=1,2.")
...
def listmatch(a, b):
    ...
    if not len(a) == len(b):
        raise ValueError(
            "Arguments must be same length, a=%s, b=%s." % (str(a), str(b))
        )
...
if __name__ == '__main__':
    ...
    for i in range(dim):
        print(
            '%16.6f %16.6f %16.6f %16.6f' % (xlower[i], xupper[i], blower[i], bupper[i])
        )
